...
function OnVoteClick( voteid, appid )
{
	...
	{
		// propmt for login
		ShowConfirmDialog( 'Выбор сообщества',
		...
		$J.post(
			...
			{sessionid: g_sessionID, voteid: voteid, appid: appid }
				...
				else
				{
					...
					ShowAlertDialog( 'Выбор сообщества', 'Благодарим за голосование!' );
				}
			}).fail( function() {
				ShowAlertDialog(
					'Выбор сообщества',
					'Возникла проблема при учете вашего голоса. Пожалуйста, попробуйте еще раз позже.'
...
function ShowVoteDialog()
{
	if ( g_AccountID )
	{
		...
		if ( g_$VoteDialog && g_$VoteDialog.length )
		{
			...
			ShowDialog( 'Выбор сообщества', g_$VoteDialog)
			.done( function() {
...
function InitTeamStandingsAJAX( rgInitialCurScores, rgInitialNextScores )
{
	...
	{
		$J.get( 'http://store.steampowered.com/promotion/summer2014teamscoreajax').done( function( data ) {
			if ( data && data[1] )
